...
exports.citizenGetById = (req, res) => {
    sequelize
        .query("SELECT * FROM citizen WHERE citizenID = '" + req.body.id + "'", {
            type: sequelize.QueryTypes.SELECT,
        })
        .then((response) => {
            res.status(200).json({
...
exports.citizenGetByName = (req, res) => {
    ...
    sequelize
        .query(
            "SELECT * FROM citizen WHERE forenames = '" +
                req.params.forenames +
                "' AND surname = '" +
                req.params.surname +
                "'",
            {
...
exports.citizenGetByOther = (req, res) => {
    console.log(req.params);
    sequelize
        .query("SELECT * FROM citizen WHERE " + req.params.column + " = '" + req.params.data + "'", {
            type: sequelize.QueryTypes.SELECT,
        })
        .then((response) => {
            res.status(200).json({
...
exports.citizenGetSingle = (req, res) => {
    console.log(req.params);
    sequelize
        .query(
            `SELECT * FROM citizen WHERE forenames = '${req.params.forenames}' AND surname = '${req.params.surname}' AND dateOfBirth = '${req.params.dateOfBirth}'`,
            {
                type: sequelize.QueryTypes.SELECT,
            }
        )
